...
const Team = () => {
    ...
    const [showForSunny, setShowForSunny] = useState(false);
    const [showForWendy, setShowForWendy] = useState(false);
    const [showForEzra, setShowForEzra] = useState(false);
    const [showForBingo, setShowForBingo] = useState(false);
    ...
    return (
        ...
        <Grid className={classes.grid} container>
            ...
            <Box className={classes.bodyContent}>
                ...
                {showForSunny ? (
                    ...
                    <Box>
                        <Button onClick={() => setShowForSunny(false)} className={classes.showLess}>Show Less</Button>
                    </Box>
                    </>
                ) : (
                    <Button onClick={() => setShowForSunny(true)} className={classes.showMore}>Read More</Button>
                )}
                </Typography>
            </Box>
            </Box>
            ...
            <Box className={classes.employee}>
                ...
                <Box className={classes.bodyContent}>
                    ...
                    {showForWendy ? (
                        <>
                        <span> We live on a meat-centric continent and I'm excited to be part of the solution. I'm passionate about helping create the  platform, society and community that I needed for support when I went vegan. 
                            I hope to see a more compassionate, conscious Africa and world where we live more sustainably, respect life and our planet and improve the world for the generations to come.
                        </span>
                        <Box>
                            <Button onClick={() => setShowForWendy(false)} className={classes.showLess}>Show Less</Button>
                        </Box>
                        </>
                    ) : (
                        <Button onClick={() => setShowForWendy(true)} className={classes.showMore}>Read More</Button>
                    )} 
                    </Typography>
                </Box>
            </Box>
        <Divider />
            <Box className={classes.employee}>
                ...
                <Typography className={classes.statement} variant="body1">
                    I'm a recent convert and believer in the vegan movement. My 117-year-old grandmother has been vegan all her life, and I always admired her for that; now I understand the benefits. I am a professional kickboxer and a second dan international blackbelt in Taekwondo, and I am dedicated to teaching young vulnerable girls how to defend themselves, empowering them by using the strong discipline instilled by martial arts.
                    {showForBingo ? (
                        <>
                        <span> I was shocked to discover the protein lie, that meat was a necessary part of every athlete's diet, and have since adopted a plant based diet. Not only has my training improved, but I have increased focus, more endurance, higher stamina and more energy.
                            ...
                            I am also working with Current Education as a Human Kinetics and Physical Education guide to several students who have their eyes set on Olympic gold.
                        </span>
                        <Box>
                            <Button onClick={() => setShowForBingo(false)} className={classes.showLess}>Show Less</Button>
                        </Box>
                        </>
                    ) : (
                        <Button onClick={() => setShowForBingo(true)} className={classes.showMore}>Read More</Button>
                    )} 
                </Typography>
                </Box>
            </Box>
            ...
            <Box className={classes.bodyContent}>
                ...
                <Typography className={classes.statement} variant="body1">
                    ...
                    {showForEzra ? (
                        <>
                        <span>  I am advising Vegan AF on operations and strategies, as I have helped many people on their plant-based journey and am aligned with the ultimate mission of making more vegans in the world.
                        ...
                        <Box>
                            <Button onClick={() => setShowForEzra(false)} className={classes.showLess}>Show Less</Button>
                        </Box>
                        </>
                    ) : (
                        <Button onClick={() => setShowForEzra(true)} className={classes.showMore}>Read More</Button>
                    )} 
                </Typography>
            </Box>
            </Box>
